...
Kafka Helm Chart 也可以直接从 OCI 仓库安装。这里我们使用 SolDevelo 提供的 Kafka Chart。

```bash
helm pull oci://registry-1.docker.io/soldevelo/kafka-chart
```
...
```bash
helm install my-kafka oci://registry-1.docker.io/soldevelo/kafka-chart --set replicaCount=3
```
...
```bash
helm install my-kafka oci://registry-1.docker.io/soldevelo/kafka-chart -f values.yaml
```
...
- [SolDevelo Kafka Chart (OCI)](https://hub.docker.com/r/soldevelo/kafka-chart)
